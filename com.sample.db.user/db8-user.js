...
function putWithId(id, number, grade, isUsed) {
    webOS.service.request("luna://com.palm.db", {
        ...
        onSuccess: function (res) {
            ...
            printLog("[putWithId] onSuccess: " + number + ", " + grade + ", " + isUsed + ", " + res.results[0].id + ", " + res.results[0].rev);
        },
        onFailure: function (res) {
            printLog("[putWithId] onFailure: " + number + ", " + grade + ", " + isUsed);
            printLog("(" + res.errorCode + ") " + res.errorText);
            return;
        }
    });
...
function get(ids) {
    webOS.service.request("luna://com.palm.db", {
        ...
        onSuccess: function (res) {
            printLog("[get] onSuccess: retrieved " + res.results.length + " object(s).");
            for (var i in res.results) {
                printLog("[get] retrieved " +  + res.results[i].number + ", " + res.results[i].grade + ", " + res.results[i].isUsed + ", " + res.results[i]._id);
            }
        },
        onFailure: function (res) {
